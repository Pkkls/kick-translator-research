...
| A18 | Zero required decisions before the first translation appears; every empty and failure state shows an actionable sentence; older settings load without loss; time to first visible translation under the ceiling in the budget file | open, with a number, and **its first clause closed** | **Zero decisions, confirmed by running it**: `translate-offline.mjs` on a `mkdtemp` profile, no settings and no reader action, ends with the translation rendered under the message. The whole chain is exercised, extension load through content script, observer, pipeline, service worker and engine adapter to the injected node, and it passes. Timing: `metrics-offline.mjs` was run for the first time in this study and gives, offline with the engine answered locally, **`e2e.cloud` p50 44 ms over 26 samples, p95 45, max 82**, of which `leg.coalesce.wait` is p50 41 and `coalesce.window` is 40. So roughly 40 of the 44 ms is the coalescing window the product chooses to wait and about 3 ms is everything else, and chapter 13's cloud p50 of 1618 ms is therefore almost entirely the provider ([4.61](E-method-log.md)). This is per message, not cold-start to first paint, so the bar's own quantity is still *not set* in [appendix G](G-budget.md) |
