...
| A6 | Heap growth over a long session | **second-half slope under 0.15 KB per message over 1200 messages, with a leaking control above 0.5**; measured 0.051 against a control at 0.914 | **Set after the measurement, which is why it cannot close the clause.** The rule here is that a bar written once the number is known is fitted to the number, and 4.107's own finding is that the obvious single quantity, the overall slope, cannot be read at all: 0.520 KB per message at 400 messages is what both a filling cap and a slow leak produce. What discriminates is the decay, so that is what the threshold is written against, and the control minimum is part of it because a flat line from a blind instrument satisfies any ceiling. The next session measures against this; this session only proposes it |
...
**9 of the 9 rows carry a number.** That sentence is read by
...
and four documents copied it (4.80). The rows are nine for seven bars: A6
states three, a weight ceiling, a per-row main-thread cost and a heap-growth
ceiling, and all three are set now.

**The file is full.** It opened at two of eight and reached eight of eight,
...
**The ninth row arrived after that**, when 4.107 measured A6's heap clause, so
*full* meant *full of the bars anyone had thought to write down*. It is worth
saying because the count had been stable long enough to read as final, and the
row that broke it came from an axis whose other two thresholds were already set.
A table is full only against the list of questions in hand.

**Two of the nine thresholds are judgements and say so in their own rows**,
