...
| A18 | Time to first visible translation, cold profile | *not set* | Chapter 13 publishes end-to-end latency, on-device p50 22 ms and cloud p50 1618 ms, but those are per message on a warm path and not first-paint on a cold profile. Playwright can do it, never run |
| A21 | Requests per unit of time under refusal, and the escalation | *not set* | Fill the storage quota and drive the product into refusal. Never run. `perChannelBudgetPerMin`, default 200, is the product's own operating limit and is not this threshold |
